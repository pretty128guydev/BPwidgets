...
interface ISideMenubarProps {
  ...
  showMenuContent?: boolean | undefined
  partnerConfig: any
...
const SideMenuBar = (props: ISideMenubarProps) => {
  ...
  const { challengesEnabled } = partnerConfig
  ...
  useEffect(() => {
    if (showChallenge && currentActiveItem === '')
      setCurrentActiveItem(t`Challenge`)
  }, [showChallenge])
  ...
  const onClickStaticItem = (label: string, component: any) => {
    collapsedSideMenu && actionSetCollapsedSideMenu(false)
    currentActiveItem !== label && setCurrentActiveItem(label)
    setShowMenuContent({ show: true, link: '', component })
  }
  ...
  return (
    <SideMenuContainer colors={colors} collapsed={collapsedSideMenu}>
      ...
      <div className="top-menu">
        {isLoggedIn && challengesEnabled && (
          <div
            className={`menu-item ${
              currentActiveItem === t`Challenge` ? 'item-active' : ''
            }`}
            onClick={() =>
              onClickStaticItem(
                t`Challenge`,
                <Challenge onClose={() => setShowMenuContent(null)} />
              )
            }
          >
            <span className="stroke-color">
              <span>
                <ChallengeIcon
                  size={20}
                  color={
                    currentActiveItem === t`Challenge`
                      ? colors.primary
                      : colors.secondaryText
                  }
                />
              </span>
              <span className="menu-item-label">{t`Challenge`}</span>
            </span>
          </div>
        )}
