...
> {
  ...
  public componentDidUpdate(
    ...
    reRenderTradingChart(instrument?.name, this.props.quote)
  }
  ...
  onInitChart = () => {
    ...
    const widgetOptions: any = {
      ...
      overrides: {
        ...
        'scalesProperties.textColor': this.props.theme?.accentHue3,
      },
    }
  ...
  onFetchCandles = async (
    ...
    if (firstDataRequest) {
      return sortBy(
        filter(data?.aggs, (item) => item.timestamp && item.open !== 0),
        ['timestamp', 'asc']
      )
    } else {
      return sortBy(
        filter(
          ...
          (item) =>
            item.timestamp &&
            item.timestamp < periodParams.to * 1000 &&
            item.open !== 0
        ),
        ['timestamp', 'asc']
      )
  ...
  DataFeed = {
      ...
      setTimeout(() => {
        ...
        if (symbolName !== instrument.name && symbolName !== '') {
          ...
          const symbol_stub = {
            ...
            minmov: 1,
            minmove2: 0,
            pricescale: this.getPriceScale(selectedSymbol?.precision || 100),
          ...
          return onSymbolResolvedCallback(symbol_stub)
        } else {
          const symbol_stub = {
    ...
    getBars: async (
      ...
      try {
        ...
        var bars = this.data.map((his: any) => {
          return {
            time: his.timestamp,
            open: his.open,
            high: his.high,
