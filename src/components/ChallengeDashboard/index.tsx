...
const ChallengeDashboardPanel = ({
  ...
  const [drawdownPercent, setDrawdownPercent] = useState<number>(
    loading ? 0 : (drawdown / Number(initialBalance)) * 100
  )
  ...
  useEffect(() => {
    /*if (!['3', '4'].includes(status))*/ {
      if (history[history.length - 1]) {
        const reCalcDailyDrawdown =
          equity - Number(history[history.length - 1].startEquity)
        if (dailyDrawdown > reCalcDailyDrawdown) {
          setDailyDrawdown(reCalcDailyDrawdown)
          setDailyDrawdownPercent(
            (reCalcDailyDrawdown / Number(initialBalance)) * 100
          )
        }
      }
    ...
    if (!loading) {
      ...
      if (activeWallet.challengeID) {
        ...
        if (success) {
          ...
          const reCalcDailyDrawdown = history[history.length - 1]
            ? Number(history[history.length - 1].minEquity) -
              Number(history[history.length - 1].startEquity)
            : 0
          setDailyDrawdown(reCalcDailyDrawdown)
        ...
        <Wrapper className="scrollable" colors={colors}>
          ...
          <div className="challenge-dashboard-container">
            ...
            <div className="dashboard-item dashboard-item-freemargin">
              <div className="dashboard-item-title">{t`Free margin`}</div>
            ...
            <div className="dashboard-item dashboard-item-marginused">
              <div className="dashboard-item-title">{t`Margin used`}</div>
              <div className="dashboard-item-value">
                {formatCurrencyFn(
                  marginUsed,
                  {
            ...
            <div className="dashboard-item dashboard-item-openpnl">
            ...
            <div className="dashboard-item dashboard-item-tradingday">
              <div className="dashboard-item-title">{t`Trading Days Limit`}</div>
              ...
              <div
                className={`trading-days-checked ${
                  maxTradingDaysReached ? 'checked-red' : 'un-checked'
                }`}
              >
                <CheckMark
                  width="20"
