...
import CircularProgress from '../../Sidebar/PositionsPanel/CircularProgress'
...
const ConfirmCloseTrade = (props: ISellbackModalProps) => {
  const { colors } = props
      ...
      <ModalHolder x={props.x} y={props.y} colors={colors}>
        ...
        <Contents>
          ...
          <ReturnBlock colors={colors}>
            <div>{t`Return`}</div>
            <span>{props.formatCurrency(parseFloat(props.amount))}</span>
          </ReturnBlock>
          <ButtonsBlock>
            <CircularProgress
            ...
            <SellbackButton
              colors={props.colors}
              onClick={() => props.actionDoSellback(props.trade, props.amount)}
            >
              {t`Confirm sellback`}
            </SellbackButton>
          </ButtonsBlock>
