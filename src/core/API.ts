...
export interface IInstrument {
  ...
  starred: boolean
  tradingConfig: InstrumentTradingConfig
  pip: number
  description: string
  lotValue: number
  ...
  contractSize: number
  closedTradeCommissionCoefficient: string
  openTradeCommissionCoefficient: string
}

export interface InstrumentTradingConfig {
  leverage: number[]
...
export interface WalletBonusDetail {
  bonusID: string
  typeID: number
  name: string
...
interface IOperationsConversionChain {
  instrumentID: number
  action: string
}

export interface IConversionChain {
  needsConversion: boolean
  operations: IOperationsConversionChain[]
}
...
class API {
  ...
  fetchInstruments = async (): Promise<IInstrument[]> => {
    ...
    if (data.success) {
      return data.data as IInstrument[]
    }
    return []
  }

  fetchTheme = async (): Promise<IInstrument[]> => {
  ...
  signIn = async (
    ...
    try {
      ...
      console.log('Debug ~ file: API.ts:505 ~ API ~ err:', err)
  ...
  fetchWalletDetails = async () => {
    try {
       ...
       * todo: move to user
       */
      const url = '/ajax/user/get-wallets'
      const params = {
        bonuses: 1,
      }
      const { data } = await this.instance.get(url, { params })
  ...
  fetchDashboard = async (walletID: number, timezoneOffset: number) => {
    const url = '/ajax/user/get-wallet-stats'
    const params = { timezoneOffset, walletID }
  ...
  fetchChallengeDashboard = async (challengeID: number) => {
    const url = '/ajax/user/get-challenge'
  ...
  fetchOptionPrices = async (instrumentID: number) => {
  ...
  closeTrades = async (trades: number[][]) => {
  ...
  cancelTrade = async (params: any) => {
    const url = '/ajax/user/cancel-order'
  ...
  editTrade = async (params: any) => {
    const url = '/ajax/user/edit-trade'
    const formData = new FormData()
    formData.append('walletID', `${params.walletID}`)
    formData.append('platformID', `${params.platformID}`)
    formData.append('tradeID', `${params.tradeID}`)
    formData.append('appId', params.appId)

    if (params.stopLoss !== undefined)
      formData.append('stopLoss', params.stopLoss)
    if (params.stopLossPrice !== undefined)
      formData.append('stopLossPrice', params.stopLossPrice)
    if (params.takeProfit !== undefined)
      formData.append('takeProfit', params.takeProfit)
    if (params.takeProfitPrice !== undefined)
      formData.append('takeProfitPrice', params.takeProfitPrice)
    try {
   ...
   */
  getConversionChain = async (instrumentID: number, walletID: number) => {
    try {
      const url = `/ajax/instrument/get-conversion-chain?instrumentID=${instrumentID}&walletID=${walletID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log(
        'Debug ~ file: API.ts:1076 ~ API ~ getConversionChain ~ err:',
        err
      )
    }
  }
  ...
  /**
   * Get Conversion Chain Take Profit
   */
  getConversionChainProfit = async (instrumentID: number, walletID: number) => {
    try {
      const url = `/ajax/instrument/get-conversion-chain-profit?instrumentID=${instrumentID}&walletID=${walletID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log(
  ...
  /**
   * Get Default View Units - Lots
   */
  getDefaultTradeBlockView = async (siteID: number) => {
    try {
      const url = `ajax/index/get-default-trade-block-view?siteID=${siteID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log(
        'Debug ~ file: API.ts:1119 ~ API ~ getDefaultTradeBlockView ~ err:',
        err
      )
    }
  }
  ...
  /**
   * Get active challenges info
   */
  getActiveChallengesInfo = async () => {
    try {
      const url = `ajax/user/get-active-challenge-packages-info`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log('Debug ~ API ~ getActiveChallengesInfo ~ err:', err)
    }
  }
   ...
   */
  addChallengeToUser = async (packageConfigID: string) => {
    try {
      const url = `ajax/user/add-challenge-package-to-user?packageConfigID=${packageConfigID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log('Debug ~ API ~ addChallengeToUser ~ err:', err)
    }
  }

  /**
   * Get challenge payment link
   */
  getChallengePaymentLink = async (
    challengePackageID: string,
    amount: number,
    walletID: number
  ) => {
    try {
      const url = `ajax/user/get-challenge-payment-link?challengePackageID=${challengePackageID}&amount=${amount}&walletID=${walletID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log('Debug ~ API ~ getChallengePaymentLink ~ err:', err)
    }
  }

  /**
   * Get challenge crypto payment link
   */
  getChallengeCryptoPaymentLink = async (
    challengePackageID: string,
    amount: number,
    walletID: number
  ) => {
    try {
      const url = `ajax/user/get-challenge-crypto-payment-link?challengePackageID=${challengePackageID}&amount=${amount}&walletID=${walletID}`
      const resp = await this.instance.get(url)
      return resp.data
    } catch (err) {
      console.log('Debug ~ API ~ getChallengePaymentLink ~ err:', err)
